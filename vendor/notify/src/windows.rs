...
fn start_read(
    ...
    unsafe {
        ...
        if ret == 0 {
            ...
            // ALLOYFS PATCH (vendored): a failed (re-)arm ends the watch and
            // previously did so in silence — `start_read` returns (), so not
            // even the initial `watch()` caller learned nothing was armed.
            emit_event(
                &request.event_handler,
                Err(Error::generic(&format!(
                    "ReadDirectoryChangesW could not be armed for {} (error {}); the watch is dropped",
    ...
    match error_code {
        ...
        ERROR_ACCESS_DENIED => {
            ...
            if !request.data.dir.exists() {
                // ALLOYFS PATCH (vendored): same contract as the arm below —
                // a watch that ends reports itself instead of dying silently.
                emit_event(
                    &request.event_handler,
                    Err(Error::generic(&format!(
                        "watched directory {} is gone; the watch is dropped",
                        request.data.dir.display()
                    ))),
                );
                request.unwatch();
        ...
        ERROR_SUCCESS => {
            // ALLOYFS PATCH (vendored): ReadDirectoryChangesW signals buffer
            // overflow as a SUCCESSFUL completion with zero bytes written —
            // changes happened, more than the buffer could describe, and
            // WHICH ones is unknowable. The buffer holds no valid entries in
            // that case, so the parse below would read stale bytes. Re-arm
            // the watch and report the gap as a `Rescan`-flagged event, the
            // same contract the inotify backend honours on Q_OVERFLOW.
            if bytes_written == 0 {
                start_read(
                    &request.data,
            ...
            // Success with data, continue to handle the event
        }
        _ => {
            ...
            // ALLOYFS PATCH (vendored): tell the event handler before the
            // watch dies — a watch dropped in silence leaves the consumer
            // trusting a stream that has stopped describing anything.
            emit_event(
                &request.event_handler,
                Err(Error::generic(&format!(
                    "ReadDirectoryChangesW failed for {} (error {}); the watch is dropped",
                    request.data.dir.display(),
                    error_code
                ))),
            );

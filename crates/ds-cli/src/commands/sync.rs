//! `drive-sync sync <url>/<export> <dir>` — bidirectional real-directory
...
pub async fn run(
    ...
    let policy: ds_client::ConflictPolicy =
    ...
    let opts = ds_client::SyncOptions {
        excludes,
        data_dir,
    ...
    let engine = ds_client::SyncEngine::start(conn, &export, &dir, opts).await?;

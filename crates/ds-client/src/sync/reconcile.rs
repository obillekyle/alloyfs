...
use ds_proto::{FileKind, Request, Response};
...
pub fn snapshot_local(root: &Path, exclude: &ExcludeSet) -> std::io::Result<Snapshot> {
    ...
    while let Some(dir) = stack.pop() {
        for item in std::fs::read_dir(&dir)? {
            let item = item?;
            let Some(rel) = ds_common::coalesce::rel_of(root, &item.path()) else {
                continue;
            };
        ...
        loop {
            let resp = conn
                .request(Request::Readdir {
                    path: ds_proto::RelPath(dir.clone()),
                    cursor,
                })
                .await??;
            ...
            for e in entries {
                ...
                if exclude.is_excluded(&ds_proto::RelPath(path.clone())) || super::is_internal(&path) {
                    continue;
                }
                match e.attr.kind {
                    FileKind::Dir => {

...
use ds_proto::{Attr, OpenFlags, RelPath, Request, Response, DATA_CHUNK};
use ds_transport::MuxConnection;
...
pub(crate) async fn download_to(conn: &MuxConnection, rel: &str, staging: &Path) -> Result<Attr, FsError> {
    ...
    let f = std::fs::OpenOptions::new()
        .write(true)
        .open(staging)
        .map_err(|_| ds_proto::ErrorCode::Io)?;
    f.set_modified(attr.mtime).map_err(|_| ds_proto::ErrorCode::Io)?;
    Ok(attr)
}
...
async fn fetch_body(conn: &MuxConnection, fh: u64, size: u64, staging: &Path) -> Result<(), FsError> {
    ...
    drop(stream);
    std::fs::write(staging, &out).map_err(|_| ds_proto::ErrorCode::Io)?;
    Ok(())
}
...
pub(crate) async fn upload(conn: &MuxConnection, rel: &str, full: &Path) -> Result<Attr, FsError> {
    let data = std::fs::read(full).map_err(|_| ds_proto::ErrorCode::Io)?;
    ...
    for (i, chunk) in data.chunks(DATA_CHUNK as usize).enumerate() {
        ...
        match resp {
            ...
            _ => {
                result = Err(ds_proto::ErrorCode::Io.into());
                break;
            }
        }
    }
        ...
        .await??
    {
        ...
        _ => Err(ds_proto::ErrorCode::Io.into()),
    }
}

...
/// itself stays alive for the duration of the mount (ds-cli guarantees that).
struct WinFspFs {
    fs: Arc<RemoteFs>,
    volume_label: String,
    ...
    pending_events: Arc<std::sync::Mutex<Vec<ds_proto::FsEvent>>>,
}
...
#[derive(Clone)]
pub struct EventSink(Arc<std::sync::Mutex<Vec<ds_proto::FsEvent>>>);
...
fn notify_name(rel: &ds_proto::RelPath) -> Vec<u16> {
...
fn emit_one(notifier: &winfsp::notify::Notifier, path: &ds_proto::RelPath, action: u32, filter: u32) {
...
impl winfsp::notify::NotifyingFileSystemContext<Vec<ds_proto::FsEvent>> for WinFspFs {
    ...
    fn notify(&self, events: Vec<ds_proto::FsEvent>, notifier: &winfsp::notify::Notifier) {
...
/// running; the caller decides when to `unmount` (ds-cli waits for Ctrl-C).
///
...
pub fn mount(fs: Arc<RemoteFs>, mountpoint: &str, volume_label: &str) -> anyhow::Result<MountedDrive> {
    ...
    let mut host = FileSystemHost::<WinFspFs>::new_with_timer::<Vec<ds_proto::FsEvent>, 100>(

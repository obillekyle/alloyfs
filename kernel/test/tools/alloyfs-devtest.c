...
/*
 * alloyfs-devtest — negative tests and a fuzzer for /dev/alloyfs.
 *
 ...
 * enumeration missed.
 *
 *   alloyfs-devtest --neg          one line per case: RESULT <name> <want> <got>
 *   alloyfs-devtest --fuzz N [seed]
 *   alloyfs-devtest --race N
 *
 * Build: gcc -static -Os -pthread -o alloyfs-devtest alloyfs-devtest.c
 */
#include <errno.h>
#include <fcntl.h>
#include <pthread.h>
...
static void mk(struct alloyfs_out_header *h, unsigned long long unique)
{
	memset(h, 0, sizeof(*h));
	h->len = sizeof(*h);
	h->error = 0;
...
{
	struct alloyfs_out_header h;
	char buf[512];
	char rbuf[sizeof(struct alloyfs_in_header) + ALLOYFS_MAX_PAYLOAD];
	void *page, *unmapped;
	size_t n;
	...
	if (page != MAP_FAILED) {
		...
		result("write_straddles_unmapped", EFAULT,
		       wr((char *)page + 4096 - 8,
			  sizeof(struct alloyfs_out_header) + 16));
		munmap(page, 4096);
	}
}
...
static void fuzz(long iterations)
{
	...
	for (i = 0; i < iterations; i++) {
		size_t len = rnd() % (sizeof(struct alloyfs_out_header) + 4096);
		...
		if (len < sizeof(struct alloyfs_out_header))
			len = sizeof(struct alloyfs_out_header);

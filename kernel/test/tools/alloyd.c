...
/*
 * alloyd — a minimal AlloyFS daemon for the test harness.
 *
 * Serves a small mutable tree over /dev/alloyfs so the kernel module can be
 ...
 *
 *   alloyd [--fd N] [--ctl PATH] [--die-after N] [--hang-on OP] [--corrupt-after N]
 *
 ...
 *
 * Build: gcc -static -Os -o alloyd alloyd.c
 */
#include <errno.h>
#include <fcntl.h>
#include <poll.h>
...
static void fill_attr(struct alloyfs_attr *a, const struct node *n)
{
	memset(a, 0, sizeof(*a));
	a->nodeid = n->nodeid;
	a->mode = n->mode;
...
{
	char buf[sizeof(struct alloyfs_out_header) + ALLOYFS_MAX_PAYLOAD];
	struct alloyfs_out_header *h = (void *)buf;

	if (len > ALLOYFS_MAX_PAYLOAD)
		len = ALLOYFS_MAX_PAYLOAD;
	h->len = sizeof(*h) + len;
	h->error = error;
	h->unique = unique;
	if (len)
...
{
	char buf[sizeof(struct alloyfs_out_header) + sizeof(struct alloyfs_notify_entry) +
		 2 * (ALLOYFS_MAX_NAME + 1)];
	struct alloyfs_out_header *h = (void *)buf;
	struct alloyfs_notify_entry *e = (void *)(buf + sizeof(*h));
	char *p = (char *)(e + 1);
	...
	e->parent2 = parent2;
	e->size = size;
	e->flags = isdir ? ALLOYFS_NOTIFY_F_ISDIR : 0;
	e->namelen = nl;
	e->name2len = nl2;
	if (nl)
		memcpy(p, name, nl);
	...
	if (write(fd, buf, h->len) < 0) {
		fprintf(stderr, "alloyd: notify: %s\n", strerror(errno));
		return -1;
	}
	return 0;
}
...
static void handle_ctl(int fd, char *line)
{
	...
	if (!strcmp(cmd, "create") || !strcmp(cmd, "mkdir")) {
		...
		notify(fd, ALLOYFS_NOTIFY_CREATE, parent, a2, 0, NULL, 0, isdir);
	} else if (!strcmp(cmd, "modify")) {
		n = find_child(parent, a2, strlen(a2));
		if (!n)
			return;
		snprintf(n->data, sizeof(n->data), "%s", a3 ? a3 : "");
		notify(fd, ALLOYFS_NOTIFY_MODIFY, parent, a2, 0, NULL, strlen(n->data), 0);
	} else if (!strcmp(cmd, "attrib")) {
		notify(fd, ALLOYFS_NOTIFY_ATTRIB, parent, a2, 0, NULL, 0, 0);
	} else if (!strcmp(cmd, "delete")) {
		n = find_child(parent, a2, strlen(a2));
		if (!n)
			return;
		n->alive = 0;
		notify(fd, ALLOYFS_NOTIFY_DELETE, parent, a2, 0, NULL, 0,
		       S_ISDIR(n->mode));
		...
		notify(fd, ALLOYFS_NOTIFY_RENAME, parent, a2, parent2, a4, 0, isdir);
	}
}

static void serve_request(int fd, char *req, ssize_t n, int *served)
{
	struct alloyfs_in_header *h = (void *)req;
	...
	if (die_after >= 0 && *served >= die_after) {
		fprintf(stderr, "alloyd: dying after %d requests\n", *served);
		_exit(0);	/* no close(): exercise abrupt death */
	}
	(*served)++;

	switch (h->opcode) {
	case ALLOYFS_OP_LOOKUP: {
		struct node *n2;
		struct alloyfs_attr a;
	...
	case ALLOYFS_OP_READDIR: {
		char payload[ALLOYFS_MAX_PAYLOAD];
		unsigned int used = 0;
		long long first;
		int idx = 0;
		...
		for (int i = 0; i < MAX_NODES; i++) {
			...
			entry = ALLOYFS_DIRENT_SIZE(namelen);
	...
	/* ------------------------------------------------ stage 4: mutations */
	case ALLOYFS_OP_CREATE:
	case ALLOYFS_OP_MKDIR: {
		const struct alloyfs_create_in *ci = (const void *)name;
		const char *nm = (const char *)(ci + 1);
		int namelen = (int)(h->len - sizeof(*h) - sizeof(*ci));
		char tmp[ALLOYFS_MAX_NAME + 1];
		struct alloyfs_attr a;
		struct node *n2;

		if (namelen <= 0 || namelen > ALLOYFS_MAX_NAME) {
	...
	case ALLOYFS_OP_UNLINK:
	case ALLOYFS_OP_RMDIR: {
		int namelen = (int)(h->len - sizeof(*h));
	...
	case ALLOYFS_OP_RENAME: {
		const struct alloyfs_rename_in *ri = (const void *)name;
		const char *from = (const char *)(ri + 1);
		const char *to = from + ri->namelen;
		struct node *n2, *victim;
		char tmp[ALLOYFS_MAX_NAME + 1];

		if (ri->namelen > ALLOYFS_MAX_NAME || ri->newnamelen > ALLOYFS_MAX_NAME) {
			reply(fd, h->unique, -EINVAL, NULL, 0);
			break;
		}
	...
	case ALLOYFS_OP_WRITE: {
		struct node *n2 = find_node(h->nodeid);
		const char *data = name;
		unsigned int len = h->len - sizeof(*h);
		struct alloyfs_write_out wo;
	...
	case ALLOYFS_OP_SETATTR: {
		const struct alloyfs_setattr_in *si = (const void *)name;
		struct node *n2 = find_node(h->nodeid);
		struct alloyfs_attr a;
		...
		if (si->valid & ALLOYFS_SETATTR_MODE)
			n2->mode = (n2->mode & S_IFMT) | (si->mode & 07777);
		if (si->valid & ALLOYFS_SETATTR_SIZE) {
...
int main(int argc, char **argv)
{
	...
	static char req[sizeof(struct alloyfs_in_header) + ALLOYFS_MAX_PAYLOAD];
	...
	if (fd < 0) {
		fd = open("/dev/alloyfs", O_RDWR);
		if (fd < 0) {
			fprintf(stderr, "open /dev/alloyfs: %s\n", strerror(errno));
			return 1;
		}
	}
	if (ctl_path) {

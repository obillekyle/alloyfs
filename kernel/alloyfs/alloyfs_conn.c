...
/*
 * The kernel <-> daemon transport: /dev/alloyfs.
 *
...
static void alloyfs_req_free(struct alloyfs_req *req)
{
	kvfree(req->in_buf);
	kvfree(req->out_buf);
	kfree(req);
...
static void alloyfs_req_put(struct alloyfs_req *req)
{
	if (refcount_dec_and_test(&req->refs))
		alloyfs_req_free(req);
}
 ...
 */
static void alloyfs_conn_abort(struct alloyfs_conn *conn)
{
	struct alloyfs_req *req, *tmp;
	LIST_HEAD(dead);
	...
	list_for_each_entry_safe(req, tmp, &dead, list) {
		...
		req->finished = true;
		complete(&req->done);
		alloyfs_req_put(req);	/* the queue's reference */
	}
...
{
	struct alloyfs_req *req;
	int ret;

	if (in_len > ALLOYFS_MAX_PAYLOAD || out_max > ALLOYFS_MAX_PAYLOAD)
		return -EINVAL;
...
static int alloyfs_dev_open(struct inode *inode, struct file *file)
{
	struct alloyfs_conn *conn = kzalloc(sizeof(*conn), GFP_KERNEL);
...
static int alloyfs_dev_release(struct inode *inode, struct file *file)
{
	struct alloyfs_conn *conn = file->private_data;

	alloyfs_conn_abort(conn);
	alloyfs_conn_put(conn);
	return 0;
}
 ...
 */
static ssize_t alloyfs_dev_read(struct file *file, char __user *ubuf, size_t count,
			     loff_t *ppos)
{
	struct alloyfs_conn *conn = file->private_data;
	struct alloyfs_req *req;
	 ...
	 */
	if (count < sizeof(struct alloyfs_in_header) + ALLOYFS_MAX_PAYLOAD)
		return -EINVAL;
	...
	for (;;) {
		...
		req = list_first_entry_or_null(&conn->pending, struct alloyfs_req, list);
		if (req) {
...
static ssize_t alloyfs_dev_write(struct file *file, const char __user *ubuf,
			      size_t count, loff_t *ppos)
{
	struct alloyfs_conn *conn = file->private_data;
	struct alloyfs_out_header hdr;
	struct alloyfs_req *req, *found = NULL;
	u32 payload;
	...
	if (hdr.len < sizeof(hdr) || hdr.len > count ||
	    hdr.len > sizeof(hdr) + ALLOYFS_MAX_PAYLOAD)
		return -EINVAL;
	...
	if (hdr.unique == 0) {
		...
		if (payload > ALLOYFS_NOTIFY_MAX)
			return -EINVAL;
		buf = memdup_user(ubuf + sizeof(hdr), payload);
		if (IS_ERR(buf))
			return PTR_ERR(buf);
		ret = alloyfs_notify_from_daemon(hdr.error, buf, payload);
		kfree(buf);
	...
	found->finished = true;
	complete(&found->done);
	alloyfs_req_put(found);
	return hdr.len;
}

static __poll_t alloyfs_dev_poll(struct file *file, poll_table *wait)
{
	struct alloyfs_conn *conn = file->private_data;
...
static const struct file_operations alloyfs_dev_fops = {
	.owner = THIS_MODULE,
	.open = alloyfs_dev_open,
	.release = alloyfs_dev_release,
	.read = alloyfs_dev_read,
	.write = alloyfs_dev_write,
	.poll = alloyfs_dev_poll,
	.llseek = noop_llseek,
};

static struct miscdevice alloyfs_miscdev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "alloyfs",
	.fops = &alloyfs_dev_fops,
	.mode = 0600,
};
...
struct alloyfs_conn *alloyfs_conn_from_fd(unsigned int fd)
{
	...
	if (!file)
		return NULL;
	if (file->f_op != &alloyfs_dev_fops) {
		fput(file);
		return NULL;
	}
	conn = alloyfs_conn_get(file->private_data);
	fput(file);
	return conn;
}
...
{
	return misc_register(&alloyfs_miscdev);
}

void alloyfs_conn_exit(void)
{
	misc_deregister(&alloyfs_miscdev);
}

...
async function resolveDefaultRef() {
  const KEY = 'alloyfs-docs-default-ref'
...
const CHANGELOG_KEY = 'alloyfs-docs-changelog'
 ...
 */
const RELEASES_ETAG = 'alloyfs-docs-rel-etag'
const NEWEST_SEEN = 'alloyfs-docs-newest'
 ...
 */
const RELEASES_SEEN = 'alloyfs-docs-rel-versions'
...
async function route() {
  ...
  const heading = content.querySelector('h1')?.textContent || 'AlloyFS'
  document.title = `${heading} | AlloyFS`
...
const stored = localStorage.getItem('alloyfs-theme')
...
document.getElementById('theme-btn').onclick = () => {
  ...
  localStorage.setItem('alloyfs-theme', next)
}
...
;(async () => {
  const CACHE = 'alloyfs-docs-version'
  const LIST = 'alloyfs-docs-versions'
